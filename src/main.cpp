...
{
    cl_device_id device_gpu = nullptr;
    cl_device_id device_cpu = nullptr;
    ...
    if (device_gpu != nullptr) {
        std::cout << "Type of chosen device: GPU" << std::endl;
        return device_gpu;
    } else if (device_cpu != nullptr) {
        std::cout << "Type of chosen device: CPU" << std::endl;
        return device_cpu;
    }
    throw std::runtime_error("There is no available device.");
}
...
int main()
{
    ...
    reportError(errcode_ret, __FILE__, __LINE__ - 1);
    ...
    cl_command_queue command_queue = clCreateCommandQueue(ctx, device, 0, &errcode_ret);
    reportError(errcode_ret, __FILE__, __LINE__ - 1);
    ...
    cl_mem as_gpu = clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), as.data(), &errcode_ret);
    reportError(errcode_ret, __FILE__, __LINE__ - 1);
    cl_mem bs_gpu = clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), bs.data(), &errcode_ret);
    reportError(errcode_ret, __FILE__, __LINE__ - 1);
    cl_mem cs_gpu = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY, n * sizeof(float), nullptr, &errcode_ret);
    reportError(errcode_ret, __FILE__, __LINE__ - 1);

    // Выполните 5 (реализуйте кернел в src/cl/aplusb.cl)
    ...
    cl_program program = clCreateProgramWithSource(ctx, 1, &kernel_sources_c_str, nullptr, &errcode_ret);
    reportError(errcode_ret, __FILE__, __LINE__ - 1);
    ...
    // Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            cl_event event;
            OCL_SAFE_CALL(clEnqueueReadBuffer(command_queue, cs_gpu, true, 0, n * sizeof(float), cs.data(), 0, nullptr, &event));
            OCL_SAFE_CALL(clWaitForEvents(1, &event));
            t.nextLap();
        }
        std::cout << "Result data transfer time: " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
        std::cout << "VRAM -> RAM bandwidth: " << 3 * n * sizeof(float) / t.lapAvg() / (1 << 30)  << " GB/s" << std::endl;
    }
